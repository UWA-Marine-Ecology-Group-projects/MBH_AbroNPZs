...
plot(slope.cuts)
inclProbs <- raster(paste(d.dir, "Altered_incProb_zone9.50_deployments.v3.tif", sep='/'))
plot(inclProbs)
...
straw.nums <- readRDS(paste(d.dir, "StrawmanNumbers_zones09.50_deployments.RDS", sep ='/'))
...
proj4string(legacy)
proj4string(newSites) <- proj4string(legacy)
