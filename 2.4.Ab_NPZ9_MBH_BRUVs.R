...
# Read rasters ----
ders <- stack(paste(r.dir, "Ab_NPZ09_ders.tif", sep='/'))

ab_rasters <- list()

ab_rasters$depth <- ders$Ab_NPZ09_ders.1
ab_rasters$slope <- ders$Ab_NPZ09_ders.2
ab_rasters$tpi <- ders$Ab_NPZ09_ders.3
ab_rasters$aspect <- ders$Ab_NPZ09_ders.4
...
#survArea first
npz_raster <- rasterize( x=npz9, y=r, field=npz6@data[,2], bkg.value=NA, fun="first")
...
# save data as RDS ----
saveRDS( ABDat, paste(d.dir, "ABData_forNPZ9.RDS", sep ='/'))
saveRDS( ab_rasters, paste(d.dir, "ABRasters_forNPZ9.RDS", sep='/'))
saveRDS( npz9,  paste(d.dir, "ABZones_forNPZ9.RDS", sep='/'))

# Read in data ----
# ABDat <- readRDS(paste(d.dir, "ABData_forNPZ9.RDS", sep='/'))
# ab_rasters <- readRDS(paste(d.dir,"ABRasters_forNPZ9.RDS", sep='/'))
# zones <- readRDS(paste(d.dir, "ABZones_forNPZ9.RDS", sep='/'))
zones <- npz9
...
writeRaster(inclProbs,paste(d.dir, 'inclProbs_forNPZ9.tif', sep='/'), overwrite=TRUE)
...
writeOGR(newSites, o.dir, "AB_NPZ9_BOSS", driver = "ESRI Shapefile")


### Make sure the drops are ~ 50 m apart ----

#inclProbs <- raster(paste(d.dir, "inclProbs_forNPZ9.tif", sep='/'))
...
p1_matrix <- gWithinDistance(s2, dist = 400, byid = TRUE)
diag(p1_matrix) <- NA
p1_matrix
...
IDnumber <- paste0(1:16)
s$IDnumber <- IDnumber
s


writeOGR(s, o.dir, "Ab_NPZ9_BRUVs", driver = "ESRI Shapefile", overwrite = TRUE)
