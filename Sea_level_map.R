...
# read state reserves ----
wamp <- readOGR(paste(s.dir, "AustraliaNetworkMarineParks.shp", sep='/'))
wamp
plot(wamp)
...
## Read Bathy ----
bathy <- raster(paste(r.dir, "Bathy_Abro.tif", sep='/'))
...
#e <- drawExtent()
#e
ext1 <- extent(112.128  , 115.0479 , -29.35965 ,-25.6138 )
...
map9 <- map71 + tm_shape(cw1) + tm_lines(col = 'red', lwd = 1) 
...
tmap_save(
  tm = map9,
  filename = paste(p.dir, "Abro-Sea-level.png", sep ='/'),
  #width = 2000,
  #height = 2200,
  #units = NA,
  scale = 1,
...
tmap_save(
  tm = map.leg,
  filename = paste(p.dir, "Abro-Sea-level-legend.png", sep ='/'),
  #width = 2000,
  #height = 2200,
  #units = NA,
  scale = 1,
